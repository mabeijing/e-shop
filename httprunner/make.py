...
def ensure_file_abs_path_valid(file_abs_path: Text) -> Text:
    ...
    """
    # C:\Users\ducker\PycharmProjects\projects\e-shop\test_case\search.yml
...
def convert_testcase_path(testcase_abs_path: Path) -> Tuple[Text, Text]:
    """convert single YAML/JSON testcase path to python file"""
    # C:\Users\ducker\PycharmProjects\projects\e-shop\test_case\search.yml
    # testcase_new_path = ensure_file_abs_path_valid(testcase_abs_path)

    dir_path: Path = testcase_abs_path.parent
    file_name, _ = os.path.splitext(os.path.basename(testcase_abs_path))
...
def make_testcase(testcase: Dict, dir_path: Text = None) -> Text:
    ...
    testcase_abs_path: Path = testcase["config"]["path"]
    ...
    for teststep in teststeps:
        ...
        _ref_testcase_path = __ensure_absolute(teststep["testcase"])
        ref_testcase_path: Path = Path(_ref_testcase_path)
        ...
        import_expr = f"from {ref_module_name} import test_case_{ref_testcase_cls_name} as {ref_testcase_cls_name}"
    ...
    diff_levels = len(testcase_path.parts)

    data = {
        "version": __version__,
        "testcase_path": testcase_path,
        "diff_levels": diff_levels,
        "class_name": f"test_case_{testcase_cls_name}",
...
def main_make(tests_paths: List[Path]) -> List[Text]:
    if not tests_paths:
        return []
...
if __name__ == '__main__':
    pa = Path(r'C:\Users\ducker\PycharmProjects\projects\e-shop')
    file = Path(r'C:\Users\ducker\PycharmProjects\projects\e-shop\resource\testcase\userlogin.yml')
    # print(file.relative_to(pa).parts)
    __make(file)
    print(pytest_files_run_set)
